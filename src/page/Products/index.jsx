...
function Products(props) {
  ...
  function fetchProbyCat(cat) {
    try {
      const fetchAPI = async () => {
        const response = await productApi.getProductByCategory(cat.slug);
  ...
  useEffect(() => {
    setTimeout(() => {
      if (title) {
        const tmp = product.filter(
          (item) => item.product_name.indexOf(`${title}`) >= 0
        );
        setListProduct(tmp);
      } else {
  ...
  function handleChangeCat(cat) {
    if (cat === "All") {
      ...
      catAtive.current = cat.category_name;
      fetchProbyCat(cat);
    }
  }
  ...
  return (
    <>
      <div>
        <div className="h-[60px] bg-white fixed top-0 left-0 right-0 z-[100] shadow-xl"></div>
