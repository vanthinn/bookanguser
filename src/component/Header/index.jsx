...
import {ClearAll, setCart} from '../../app/CartSlice'
...
import {setProducts} from '../../app/ProductSlice.js'
...
function Header(props) {
            ...
            try {
                const fetch = async () => {
                    ...
                    for (let i = 0; i < products.length; i++) {
                        for (let j = 0; j < response.cart_items.length; j++) {
                            if (
                                products[i].slug ===
                                response.cart_items[j].product
                            ) {
                                cartItems.push({
                                    ...products[i],
                                    cartQuantity:
                                        response.cart_items[j].quantity,
                                })
                            }
                        }
                    }
    ...
    return (
        <>
            ...
            <div
                className={` app-container w-full px-7  fixed top-0 left-0 right-0 flex items-center h-[60px] z-[200] text-black
       transition-color duration-500 ${
           navState ? 'bg-[#fff] animate-navbar shadow-xl' : 'bg-slate-100/70'
       }
       ${checknotfound ? '' : 'hidden'}`}>
                <div className='flex gap-4 lg:ml-10 '>
                    ...
                    />
                    <img
                        className='h-[92px] sm:w-[65%]  lg:w-[75%] lg:pr-8 cursor-pointer object-contain '
                ...
                <div className='w-[90%] flex sm:justify-end lg:justify-between lg:items-center '>
                    <div className='sm:hidden md:hidden lg:flex lg:items-center lg:text-base'>
                        {mainNav.map((item, index) => (
                            <div
                                key={index}
                                className={`mx-6 hover:text-yellow-600 transition-all duration-300 ease-in-out  ${
                                    index === activeNav
                    ...
                    <div className='flex items-center lg:mr-[28px] '>
                        <div className='sm:hidden lg:block relative mx-4 w-[400px]'>
                            <input
                                type='text'
                                className=' text-black w-full text-base bg-transparent border-b-[2px] px-2 py-[2px] outline-none placeholder:pl-2 placeholder:text-[15px] placeholder:text-black'
                        ...
                        {islogin && (
                            <div className='sm:hidden lg:block flex items-center relative group cursor-pointer'>
                                <span>{username}</span>
                                <span
                                    className='absolute left-[-40px] bottom-[-150%] w-[150px] border-[1px] border-gray-300 shadow-md transitions-theme opacity-0 rounded-sm bg-white text-slate-700 group-hover:bottom-[-135%] group-hover:opacity-100  '
